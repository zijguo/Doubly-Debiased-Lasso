...
### estimate_sigma
### FUNCTION: Creates an unbiased estimator of noise level based on
###
###
### INPUT: X, an nxp matrix; represents the measurements for each subject and covariate
###        Y, an nx1 matrix; the measured response for each subject
...
estimate_sigma = function(X,Y,rho=0.5,alt=FALSE){
  ...
  #two methods of sigmahat computation, the second has shown more robust results
  if(alt){
...
###           it is recommended to transform X before putting in this function
###
...
###        rhop, a double; the trim parameter for the P trim transform, default is 0.5(median)
###
...
###        Variance_alt, a boolean; FALSE to use more robust method, TRUE to use original, default FALSE
###
...
CI = function(X,Y,idx,alpha=0.05,rho=0.5,rhop=0.5,Variance_alt=FALSE){
  ...
  if(Variance_alt){
    ...
    #suggested choice for variance
    Variance = (norm(P%*%z, type ="F") / (t(z) %*% X[,idx]))^2
  }
  #(Step 7)
...
###           representing a sample with many covariates and confounding variables
###           along with
###           CURRENTLY DOES NOT COMPUTE CORRECTLY
###
### INPUT: X, an nxp matrix; represents the measurements for each subject and covariate
###        Y, an nx1 matrix; the measured response for each subject
...
###
###
### OUTPUT: a list, (a) a double, bias term for Beta
###                 (b) a double, bias term for b
###                 (c) a double, bias term for b if oracle value is known
###                 (d) a double, bias term for b if bound
###                 (e) a double, bias term for b if corrected
...
compute_bias = function(X,Y,beta,b,idx,rho=0.5,rhop=0.5){
  ...
  B_beta =(t(z) %*% P_X[,-idx] %*% row_del[-idx,])/ ((t(z)%*%P_X[,idx]))/stddev
  ...
  return_list = list("B_beta"=B_beta, "B_b" = B_b, "B_oracle" =B_b_oraclebound,"B_bound" =B_b_bound,
                     "B_corrected"=B_b_corrected)
  return(return_list)
}
